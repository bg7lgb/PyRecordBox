...
def readConfig():
    global host, port, logger, uboxlog, prefix
...
#        threading.Thread(target=self.server.serve_forever).start()
    ...
    def _setup_widgets(self):
        ...
        self.statusBar = tkinter.ttk.Label(text="", justify='left',relief='flat') 
        self.statusBar.grid(column=0, row = 3, sticky='we', columnspan=2, in_=container, pady=5)
    ...
    def dial(self):
        ...
        line = item['values'][0]
        phone_no = prefix + str(item['values'][1])
        logger.debug("phone_no %s %s" %(type(phone_no), phone_no))
...
#        self.rbox.dial(line, phone_no)
        # 从py2转py3,需要注意String的转换
        phone_no_byte = phone_no.encode()
        #logger.debug("phone_no %s %s" %(type(phone_no_byte), phone_no_byte))
        self.rbox.dial(self.lines[line-1]["uboxHnd"], phone_no_byte)
...
#        self.uboxHnd = uboxHnd
        ...
        if eventID == 1:
            ...
            self.call[1] = str(callid.value, 'utf-8')
            ...
            self.displayMessage('线路 %d 来电号码 %s' %(self.lines[uboxHnd-self.uboxHnd]["line_no"], self.call[1]))
            logger.debug("线路 %d 来电号码 %s" %(self.lines[uboxHnd-self.uboxHnd]["line_no"], self.call[1]))
